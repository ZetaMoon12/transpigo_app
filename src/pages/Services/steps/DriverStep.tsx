...
const GRUA_VEHICLE_TYPES = VEHICLE_TYPES_BY_SERVICE.GRUA_AUXILIO_VIAL;
...
export function DriverStep({ state, onChange, error }: DriverStepProps) {
    ...
    <div className="flex flex-col gap-6">
      <div>
        ...
        <p className="text-sm text-slate-500 mt-0.5">
          Asigna un conductor disponible o continúa sin asignar (podrás asignarlo después).
        </p>
      </div>
      ...
      {!loading && drivers.length === 0 && (
        <p className="text-xs text-slate-400">
          No hay conductores de grúa disponibles en este momento. Puedes continuar sin asignar.
        </p>
      )}

      {!state.selectedDriver && (
        <FormField label="Tipo de vehículo (para cotizar)" required error={error}>
          <Select
            value={state.manualVehicleType ?? ''}
            onValueChange={(v) => onChange({ manualVehicleType: (v || null) as VehicleType | null, quote: null })}
          >
            <SelectTrigger className="w-full sm:w-72">
              <SelectValue>
                {state.manualVehicleType ? VEHICLE_TYPE_LABEL[state.manualVehicleType] : 'Selecciona un tipo…'}
              </SelectValue>
            </SelectTrigger>
            <SelectContent>
              {GRUA_VEHICLE_TYPES.map((type) => (
